...
public class OrderDAO 
{
	...
	public void saveOrUpdate(Order order) throws SQLException 
	{
		if (order.getCreationDateTime() == null)
		{
			save(order);
			return;
		}
		
		final PreparedStatement statement = conn.prepareStatement("update Order set "
				+ "customerName=?, horoscopeType=?, ageOfTargetPerson=?, creationDateTime=?, "
				+ "price=?, paid=?, horoscopeFetched=?"
				+ "where orderNumber=?");
		
		statement.setString(1, order.getCustomerName());
		statement.setString(2, order.getHoroscopeType().name());
		statement.setInt(3, order.getAgeOfTargetPerson());
		statement.setDate(4, toSqlDate(order.getCreationDateTime()));
		statement.setBigDecimal(5, order.getPrice());
		statement.setBoolean(6, order.isPaid());
		statement.setBoolean(7, order.isHoroscopeFetched());	
		statement.setLong(8, order.getOrderNumber());
	...
	{
		final OrderImpl order = new OrderImpl(resultSet.getLong(1));
